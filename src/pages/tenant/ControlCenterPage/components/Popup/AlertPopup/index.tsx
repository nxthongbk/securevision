import { Box, Grid, Typography, useMediaQuery, useTheme, Tabs, Tab } from '@mui/material';
        ...
        <Box className='flex flex-col gap-6 p-4'>
          ...
          <div className='bg-[#161B29] p-3 flex gap-4 items-center'>
            <div className='flex-1'>
              <CommonInfoLocation
                info={status === 'error' ? { ...openAlertPopup, status: 'CONFIRM' } : openAlertPopup}
              />
            </div>
          </div>
  ...
  const toggleFullscreen = () => {
    ...
    if (container) {
      if (!document.fullscreenElement) {
        container.requestFullscreen().catch((err) => console.error(`Error attempting fullscreen: ${err.message}`));
      } else {
