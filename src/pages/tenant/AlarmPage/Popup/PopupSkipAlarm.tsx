...
export default function PopupSkipAlarm({
  ...
  const skipSchema = yup.object({
    reason: yup.string().required(alarmTranslate('please-select-reason'))
  });
      ...
      {isTablet ? (
        <DrawerUpdate open={open} onClose={onClose} title={title} isValid={isValid} isSuccess={isSuccess} isLoading={isPending} onSubmit={handleSkip}>
          <InputCustom
            ...
            isRequired
            label={alarmTranslate('reason')}
            control={control}
            name='reason'
            placeholder={alarmTranslate('enter-reason')}
        ...
        <DialogCustom
          open={open}
          title={alarmTranslate('skip-alert')}
          maxWidth='600px'
          handleClose={onClose}
          footer={
